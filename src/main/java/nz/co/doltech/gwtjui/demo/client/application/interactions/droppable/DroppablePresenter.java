...
public class DroppablePresenter extends Presenter<DroppablePresenter.MyView, DroppablePresenter.MyProxy>
        implements DroppableUiHandlers {
    interface MyView extends View, HasUiHandlers<DroppableUiHandlers> {
    }

    @NameToken(NameTokens.droppable)
    @ProxyCodeSplit
    interface MyProxy extends ProxyPlace<DroppablePresenter> {
    }

    @Inject
    DroppablePresenter(EventBus eventBus, MyView view, MyProxy proxy) {

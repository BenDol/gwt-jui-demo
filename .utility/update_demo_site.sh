...
if [ ! -f $TRAVIS_BUILD_DIR/target/gwt-jui-demo-*.war ]; then
...
cd $HOME
rm -rf gh-pages
git clone --quiet --branch=gh-pages https://$GH_TOKEN@github.com/BenDol/gwt-jui-demo gh-pages > /dev/null
cd gh-pages
...
# copy the new GwtJuiDemo the snapshot dir.
unzip -u $TRAVIS_BUILD_DIR/target/gwt-jui-demo-*.war -d ./snapshot/
